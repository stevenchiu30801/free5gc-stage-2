...
asn1c_make_identifier(enum ami_flags_e flags, asn1p_expr_t *expr, ...) {
	...
	if(expr) {
		...
		if(expr->spec_index != -1 && expr->_lineno && !(flags & AMI_TITLE)) {
			static char buf[32];
	...
	for(str = 0; str || nextstr; str = nextstr) {
		...
		if(str != first && !nodelimiter && !(flags & AMI_NODELIMITER) && !(flags & AMI_TITLE))
		...
		&& str == first && !nextstr && reserved_keyword(str) || flags & AMI_TITLE) {
			*p++ = toupper(*str++);
			/* Fall through */
		}
		...
		for(; *str; str++) {
			if(isalnum(*str)) {
				...
				if(flags & AMI_TITLE) {
					next_title = 1;
				} else if((flags & AMI_MASK_ONLY_SPACES)
						   && !isspace(*str)) {
					*p ++ = *str;
				} else {
					*p++ = '_';
...
const char *
asn1go_type_name(arg_t *arg, asn1p_expr_t *expr, enum tnfmt _format) {
	asn1p_expr_t *exprid = 0;
	asn1p_expr_t *top_parent;
	asn1p_expr_t *terminal = 0;
	int stdname = 0;
	...
	if(0) DEBUG("asn1go_type_name(%s: 0x%x)",
		expr->Identifier, expr->expr_type);
	...
	case A1TC_REFERENCE:
		...
		if(typename[0] == '&') {
			...
			return asn1go_type_name(&tmp, tmp.expr, _format);
		}
	...
	case TNF_CONSTYPE:	/* C type */
	case TNF_RSAFE:	/* Recursion-safe type */
		return asn1c_make_identifier(AMI_TITLE | AMI_NODELIMITER, 0,
			    typename, (char*)0);
	}
