...
c_name_impl(arg_t *arg, asn1p_expr_t *expr, int avoid_keywords) {
    ...
    abuf_str(&b_type_go_name, asn1go_type_name(arg, expr, TNF_RSAFE));
    ...
    construct_base_name(&b_title_name, expr, 0, 0, AMI_TITLE);
    ...
    names.type.go_name = b_type_go_name.buffer;
    ...
    names.title_name = b_title_name.buffer;
...
const char *
go_member_name(arg_t *arg, asn1p_expr_t *expr) {
    static abuf ab; 

    abuf_clear(&ab);
    /* NB: do not use part_name, doesn't work for -fcompound-names */
    abuf_str(&ab, c_name_impl(arg, arg->expr, 0).base_name);
    abuf_str(&ab, asn1c_make_identifier(AMI_TITLE | AMI_NODELIMITER, expr, 0));
...
const char *
go_presence_name(arg_t *arg, asn1p_expr_t *expr) {
    static abuf ab;

    abuf_clear(&ab);

    if(expr) {
        abuf_str(&ab, c_name_impl(arg, arg->expr, 0).title_name);
        abuf_str(&ab, "Present");
        abuf_str(&ab, asn1c_make_identifier(AMI_TITLE | AMI_NODELIMITER, expr, 0));
    }
    else {
        abuf_printf(&ab, "%sPresentNothing", c_name_impl(arg, arg->expr, 0).title_name);
    }
