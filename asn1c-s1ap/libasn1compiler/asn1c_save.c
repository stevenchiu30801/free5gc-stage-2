...
static int asn1c_save_compiled_output_C(arg_t *arg, const char *datadir, const char *destdir,
                                        int argc, int optc, char **argv); 
static int asn1c_save_compiled_output_Go(arg_t *arg, const char *datadir, const char *destdir,
                                         int argc, int optc, char **argv);
static int asn1c_dump_streams(arg_t *arg, asn1c_dep_chainset *, const char *, int, char **);
static int asn1c_print_streams(arg_t *arg);
static int asn1c_save_streams_C(arg_t *arg, asn1c_dep_chainset *, const char *,
                                int, char **); 
static int asn1c_save_streams_Go(arg_t *arg, asn1c_dep_chainset *, const char *,
                                 int, char **);
...
static int generate_go_preamble(arg_t *, FILE *, int optc, char **argv);
...
asn1c_save_compiled_output(arg_t *arg, const char *datadir, const char *destdir,
                           int argc, int optc, char **argv) {
    int ret;
    if(arg->flags & A1C_LANG_GO) {
        ret = asn1c_save_compiled_output_Go(arg, datadir, destdir, argc, optc, argv);
    }
    else if(arg->flags & A1C_LANG_C) {
        ret = asn1c_save_compiled_output_C(arg, datadir, destdir, argc, optc, argv);
    }
    else {
        safe_fprintf(stderr, "This output language has not implemented yet\n");
        ret = -1;
    }
    return ret;
}
...
static int
asn1c_save_compiled_output_Go(arg_t *arg, const char *datadir, const char *destdir,
                           int argc, int optc, char **argv) {
    int ret = -1;
...
static int
asn1c_save_compiled_output_C(arg_t *arg, const char *datadir, const char *destdir,
                           int argc, int optc, char **argv) {
    int ret = -1;
...
asn1c_dump_streams(arg_t *arg, asn1c_dep_chainset *deps, const char *destdir,
    ...
    if(arg->flags & A1C_PRINT_COMPILED) {
        ...
        if(arg->flags & A1C_LANG_GO) {
	    	return asn1c_save_streams_Go(arg, deps, destdir, optc, argv);
        }
        else if(arg->flags & A1C_LANG_C){
	    	return asn1c_save_streams_C(arg, deps, destdir, optc, argv);
        }
        else {
            safe_fprintf(stderr, "This output language has not implemented yet\n");
            return -1;
...
static int
asn1c_save_streams_Go(arg_t *arg, asn1c_dep_chainset *deps, const char *destdir,
                   int optc, char **argv) {
	asn1p_expr_t *expr = arg->expr;
	...
	filename = strdup(asn1c_make_identifier(AMI_TITLE | AMI_USE_PREFIX,
						expr, (char*)0));

	fp_go = asn1c_open_file(destdir, filename, ".go", &tmpname_go);
    if(fp_go == NULL) {
        return -1;
    }

	generate_go_preamble(arg, fp_go, optc, argv);

    // TODO : Some contain need to write into file
	TQ_FOR(ot, &(cs->destination[OT_IOC_TABLES].chunks), next)
		safe_fwrite(ot->buf, ot->len, 1, fp_go);
	...
	int ret = snprintf(name_buf, sizeof(name_buf), "%s%s.go", destdir, filename);
    assert(ret > 0 && ret < (ssize_t)sizeof(name_buf));

	if(identical_files(name_buf, tmpname_go)) {
		go_retained = " (contents unchanged)";
		unlink(tmpname_go);
	} else {
		if(rename(tmpname_go, name_buf)) {
			unlink(tmpname_go);
			perror(tmpname_go);
			free(tmpname_go);
			return -1;
	...
	safe_fprintf(stderr, "Compiled %s%s\n",
             name_buf, go_retained);
...
static int
asn1c_save_streams_C(arg_t *arg, asn1c_dep_chainset *deps, const char *destdir,
                   int optc, char **argv) {
    asn1p_expr_t *expr = arg->expr;
	compiler_streams_t *cs = expr->data;
	out_chunk_t *ot;
...
static int 
generate_go_preamble(arg_t *arg, FILE *fp, int optc, char **argv) {
    safe_fprintf(fp,
    "package ngapType\n"
    "\n"
