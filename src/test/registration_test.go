...
var ranConfig string = "rancfg.conf"
...
func TestRegistration(t *testing.T) {
	...
	// set MongoDB
	MongoDBLibrary.SetMongoDB("free5gc", "mongodb://"+test.TestConfig.MongoAddr+":27017")
	...
	sendMsg, err = test.GetPDUSessionResourceSetupResponse(ue.AmfUeNgapId, ue.RanUeNgapId, test.TestConfig.IpAddr)
	assert.Nil(t, err)
	_, err = conn.Write(sendMsg)
	assert.Nil(t, err)
	...
	// RAN connect to UPF
	upfConn, err := connectToUpf(test.TestConfig.IpAddr, upfAddr, 2152, 2152)
	assert.Nil(t, err)
